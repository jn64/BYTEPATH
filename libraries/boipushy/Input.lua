			...
			for _, key in ipairs(self.binds[action]) do
				if self.state[key] and not self.prev_state[key] then
					if
						(
							love.timer.getTime()
							- ((self.sequences[sequence_key] and self.sequences[sequence_key].last_pressed) or 0)
						) <= delay
					then
...
function Input:down(action, interval, delay)
		...
		for _, key in ipairs(self.binds[action]) do
			if self.state[key] and not self.prev_state[key] then
				self.repeat_state[key] =
					{ pressed_time = love.timer.getTime(), delay = delay, interval = interval, delay_stage = true }
				return true
	...
	elseif action and not interval and not delay then
		for _, key in ipairs(self.binds[action]) do
			if
				((not Input:notKey(key)) and love.keyboard.isDown(key)) or love.mouse.isDown(key_to_button[key] or 0)
			then
				return true
			end
