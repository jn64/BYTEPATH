function sound()
	...
	for i = 1, 28 do
		register("keystroke_" .. i, "static", { "master", "sfx", "keyboard" })
	end
...
function register(name, source_type, tags)
	if not sources[name] then
		sources[name] = {
			source = love.audio.newSource("resources/sounds/" .. name .. ".ogg", source_type),
			type = source_type,
			tags = tags,
		}
	end
...
function play(name, opts)
	...
	if not sources[name] then
		error("unregistered source " .. name)
	end
	if sources[name].played_this_frame then
		return
	end
	...
	if source.type == "static" then
		...
		for _, tag in ipairs(source.tags) do
			volume = volume * (tags[tag].volume or 1)
		end
		...
		local volume = opts.volume or 1
		for _, tag in ipairs(sources[name].tags) do
			volume = volume * (tags[tag].volume or 1)
		end
...
function fadeIn(name, opts)
	...
	if not opts.duration then
		error("fadeIn() needs a 'duration' argument in the options table")
	end
	local time = 0
...
function playRandomSong()
	if currently_playing_song then
...
function fadeOutComputerBackground()
	if not computer_background_source then
		return
	end
