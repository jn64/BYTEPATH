...
function Console:new()
	...
	self.timer:every(0.05, function()
		...
		table.insert(
			self.glitches,
			GlitchDisplacementC(
				love.math.random(0, gw),
				love.math.random(0, gh),
				love.math.random(16, 48),
	...
	self.timer:every({ 0.1, 0.2 }, function()
...
function Console:update(dt)
	...
	if self.bytepath_main_active then
		if input:pressed("up") then
			...
			if self.bytepath_main_selection_index == 0 then
				self.bytepath_main_selection_index = #self.bytepath_main_selection_widths
			end
			playMenuSwitch()
		end
		...
		if input:pressed("down") then
			self.bytepath_main_selection_index = self.bytepath_main_selection_index + 1
			if self.bytepath_main_selection_index == 7 then
		...
		if input:pressed("return") then
			...
			for i = 1, #command do
				...
				self.timer:after(0.025 * i, function()
	...
	if not self.bytepath_main then
...
function Console:addLine(after, text, duration, swaps)
	self.timer:after(after, function()
		...
		if self.line_y > gh then
			camera:lookAt(camera.x, camera.y + 12)
		end
	end)
end
...
function Console:bytepathMain(delay)
	...
	self:addLine(
		delay + 0.04,
		":: " .. os.date("%a %b %d ") .. tostring(tonumber(os.date("%Y")) + 1000) .. os.date(" %X ") .. "on ;VCON1,"
	)
	...
	self:addLine(
		delay + 0.26,
		"Your objective is to escape this terminal by reaching difficulty 40 and finding all keys."
	)
	...
	self.timer:after(delay, function()
		...
		self.bytepath_main_selection_widths = {
			self.font:getWidth("escape"),
			self.font:getWidth("start"),
			self.font:getWidth("help"),
			self.font:getWidth("classes"),
			self.font:getWidth("device"),
			self.font:getWidth("passives"),
		}
		if loop > 0 then
			self.bytepath_main_y = self.line_y + 13 * 12
		else
			self.bytepath_main_y = self.line_y + 13 * 12 - 12
		end
		self.timer:after(0.38, function()
			self.bytepath_main_active = true
		end)
	end)
end
...
function Console:bytepathMain2()
	...
	self.timer:after(delay, function()
		...
		self.timer:after(0.30, function()
			self.bytepath_main_active = true
		end)
	end)
end
...
function Console:bytepathIntro()
	...
	self:addLine(
		delay + 4.80,
		"[  ;WAIT,  ] allocating user and session ("
			.. self:getRandomArchWord()
			.. ") ["
			.. self:getRandomArchWord()
			.. "]",
		1,
		{ { ";WAIT,", " <OK> " }, { "allocating", "allocated" } }
	)
	self:addLine(
		delay + 4.81,
		"[  ;WAIT,  ] allocating system (" .. self:getRandomArchWord() .. ") [" .. self:getRandomArchWord() .. "]",
		1.1,
		{ { ";WAIT,", " <OK> " }, { "allocating", "allocated" } }
	)
	self:addLine(
		delay + 5.00,
		"[  ;WAIT,  ] mounting page file system ("
			.. self:getRandomArchWord()
			.. ") ["
			.. self:getRandomArchWord()
			.. "]",
		1,
		{ { ";WAIT,", " <OK> " }, { "mounting", "mounted" } }
	)
	self:addLine(
		delay + 5.01,
		"[  ;WAIT,  ] starting load core modules ("
			.. self:getRandomArchWord()
			.. ") ["
			.. self:getRandomArchWord()
			.. "]",
		0.8,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 5.2,
		"[  ;WAIT,  ] starting (" .. self:getRandomArchWord() .. ") devices",
		0.4,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 5.60,
		"[  ;WAIT,  ] starting virtual console",
		1.2,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 5.61,
		"[  ;WAIT,  ] mounting (" .. self:getRandomArchWord() .. ") message queue file system",
		1.4,
		{ { ";WAIT,", " <OK> " }, { "mounting", "mounted" } }
	)
	...
	self:addLine(
		delay + 5.81,
		"[  ;WAIT,  ] mounting temporary directory ("
			.. self:getRandomArchWord()
			.. ") ["
			.. self:getRandomArchWord()
			.. "]",
	...
	self:addLine(
		delay + 6.2,
		"[  ;WAIT,  ] starting root and core file systems [" .. self:getRandomArchWord() .. "]",
		1.4,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 6.40,
		"[  ;WAIT,  ] starting random seed [" .. self:getRandomArchWord() .. "]",
		0.4,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 6.41,
		"[  ;WAIT,  ] creating system users",
		0.1,
		{ { ";WAIT,", " <OK> " }, { "creating", "created" } }
	)
	self:addLine(
		delay + 6.42,
		"[  ;WAIT,  ] creating static nodes",
		0.6,
		{ { ";WAIT,", " <OK> " }, { "creating", "created" } }
	)
	self:addLine(
		delay + 6.80,
		"[  ;WAIT,  ] mounting configuration file system [" .. self:getRandomArchWord() .. "]",
		0.4,
		{ { ";WAIT,", " <OK> " }, { "mounting", "mounted" } }
	)
	self:addLine(
		delay + 6.81,
		"[  ;WAIT,  ] applying core variables [" .. self:getRandomArchWord() .. "]",
		0.1,
		{ { ";WAIT,", " <OK> " }, { "applying", "applied" } }
	)
	self:addLine(
		delay + 6.82,
		"[  ;WAIT,  ] starting (" .. self:getRandomArchWord() .. ") (" .. self:getRandomArchWord() .. ") manager",
		1.2,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	...
	self:addLine(
		delay + 8.40,
		"[  ;WAIT,  ] starting primary message system path [" .. self:getRandomArchWord() .. "]",
		5.6,
		{ { ";WAIT,", "@FAIL#" }, { "starting", "could not start" } }
	)
	self:addLine(
		delay + 9.80,
		"[  ;WAIT,  ] starting auxiliary message system path [" .. self:getRandomArchWord() .. "]",
		4.2,
		{ { ";WAIT,", "@FAIL#" }, { "starting", "could not start" } }
	)
	self:addLine(
		delay + 11.00,
		"[  ;WAIT,  ] solving for optimal path [" .. self:getRandomArchWord() .. "]",
		3.0,
		{ { ";WAIT,", "@FAIL#" }, { "solving", "could not solve" } }
	)
	self:addLine(
		delay + 12.00,
		"[  ;WAIT,  ] reaching target (" .. self:getRandomArchWord() .. ")",
		2.0,
		{ { ";WAIT,", "@FAIL#" }, { "reaching", "could not reach" } }
	)
	self:addLine(
		delay + 12.01,
		"[  ;WAIT,  ] reaching target (" .. self:getRandomArchWord() .. ")",
	...
	self:addLine(
		delay + 12.05,
		"[  ;WAIT,  ] reaching core target (" .. self:getRandomArchWord() .. ")",
		2.0,
		{ { ";WAIT,", "@FAIL#" }, { "reaching", "could not reach" } }
	)
	self:addLine(
	...
	for i = 1, 8 do
		self:addLine(
			delay + 14.55 + i * 0.05,
			":: could not reach node @(" .. self:getRandomArchWord() .. ")# @[" .. self:getRandomArchWord() .. "]#"
		)
	end
	...
	self:addLine(
		delay + 16.20,
		"[  ;WAIT,  ] starting safe user and session ("
			.. self:getRandomArchWord()
			.. ") ["
			.. self:getRandomArchWord()
			.. "]",
	...
	self:addLine(
		delay + 17.80,
		"[  ;WAIT,  ] starting primary repair system [" .. self:getRandomArchWord() .. "]",
		2.0,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
	self:addLine(
		delay + 17.90,
		"[  ;WAIT,  ] starting auxiliary repair system [" .. self:getRandomArchWord() .. "]",
		2.0,
		{ { ";WAIT,", " <OK> " }, { "starting", "started" } }
	)
