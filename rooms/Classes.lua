...
function Classes:new()
	...
	self.class_colors = {
		...
		["Tanker"] = hp_color,
		["Runner"] = boost_color,
		["Cycler"] = default_color,
		["Buster"] = ammo_color,
		["Buffer"] = ammo_color,
		["Berserker"] = ammo_color,
		["Shielder"] = hp_color,
		["Regeneer"] = hp_color,
		["Recycler"] = hp_color,
		["Absorber"] = boost_color,
		["Turner"] = boost_color,
		["Driver"] = boost_color,
		["Swapper"] = default_color,
		["Barrager"] = default_color,
		["Seeker"] = default_color,
		["Repeater"] = ammo_color,
		["Launcher"] = ammo_color,
		["Panzer"] = hp_color,
		["Reserver"] = hp_color,
		["Deployer"] = boost_color,
		["Booster"] = boost_color,
		["Processor"] = default_color,
		["Gambler"] = default_color,
		["Discharger"] = boost_color,
		["Hoamer"] = skill_point_color,
		["Splitter"] = boost_color,
		["Bouncer"] = default_color,
		["Blaster"] = default_color,
		["Raider"] = skill_point_color,
		["Waver"] = ammo_color,
		["Bomber"] = hp_color,
		["Zoomer"] = boost_color,
		["Racer"] = boost_color,
...
function Classes:update(dt)
	...
	if rank >= 5 and rank <= 6 then
		n = 2
	end
	if rank >= 7 and rank <= 8 then
		n = 4
	end
	if rank == 9 then
		n = 4
	...
	if input:pressed("left") then
		...
		if
			...
			or self.selection_index == n
			or self.selection_index == 2 * n
			or self.selection_index == 3 * n
		then
			self.selection_index = self.selection_index + n
			if self.selection_index > #self.classes[rank] then
				self.selection_index = #self.classes[rank]
			end
		end
		playMenuSwitch()
		self:changedIndex()
	...
	if input:pressed("right") then
		self.selection_index = self.selection_index + 1
		if self.selection_index == n + 1 or self.selection_index == 2 * n + 1 or self.selection_index == 3 * n + 1 then
			self.selection_index = self.selection_index - n
		elseif self.selection_index > #self.classes[rank] then
			if rank >= 1 and rank <= n then
				self.selection_index = 1
			elseif rank >= n + 1 and rank <= 2 * n then
				self.selection_index = n + 1
			elseif rank >= 2 * n + 1 and rank <= 3 * n then
				self.selection_index = 2 * n + 1
			elseif rank >= 3 * n + 1 and rank <= #self.classes[rank] then
				self.selection_index = 3 * n + 1
			end
		end
		playMenuSwitch()
		self:changedIndex()
...
function Classes:draw()
	...
	for i = 1, rank do
		...
		if not classes[i] then
			drawNormalButton(8 + 24 + 56, y, 64, 16, "COST: " .. tostring(rank * 5) .. "SP", skill_point_color)
		end
		if classes[i] then
			drawNormalButton(8 + 24 + 48, y, 48, 16, classes[i], self.class_colors[classes[i]])
		else
			for j = 1, #self.classes[i] do
				if self.selection_index == j then
					drawInvertedButton(
						92 + 60 + ((j - 1) % n) * 48,
						y + (math.ceil(j / n) - 1) * 24,
						48,
						16,
						self.classes[i][j]
					)
					...
					drawNormalButton(
						...
						16,
						self.classes[i][j],
						self.class_colors[self.classes[i][j]]
					)
				end
			end
		end
...
function Classes:destroy() end

function Classes:changedIndex()
	self.timer:every(0.035, function()
		self.visible = not self.visible
	end, 5)
	self.timer:after(0.035 * 5 + 0.02, function()
		self.visible = true
	end)
end
