...
function Stage:new()
	...
	input:bind("f7", function()
		self.area:addGameObject("Glitcher", 0, 0)
	end)
	...
	if run <= 3 then
		self.tutorial = Tutorial(gw / 2, gh / 2)
	end
...
function Stage:getInfoTextPosition(x, y)
	...
	local runs = 0
	local function snap(v, x)
		return math.floor(v / x) * x
	end
	local grid_x, grid_y = math.floor(snap(x, 60) / 60) + 1, math.floor(snap(y, 15) / 15) + 1
	if grid_x > 2 then
		grid_x = grid_x + love.math.random(0, 2) - 2
	end
...
function Stage:update(dt)
	...
	if self.scorescreen then
		self.scorescreen_object:update(dt)
	end

	if not self.scorescreen then
...
function Stage:draw()
	...
	love.graphics.print(
		...
		1,
		1,
		math.floor(self.font:getWidth("AMMO") / 2),
		math.floor(self.font:getHeight() / 2)
	)
	...
	love.graphics.print(
		...
		1,
		1,
		math.floor(self.font:getWidth(ammoText) / 2),
		math.floor(self.font:getHeight() / 2)
	)
	love.graphics.setColor(color255To1(255, 255, 255))
	...
	love.graphics.print(
		...
		1,
		1,
		math.floor(self.font:getWidth("BOOST") / 2),
		math.floor(self.font:getHeight() / 2)
	)
	...
	love.graphics.print(
		...
		1,
		1,
		math.floor(self.font:getWidth(boostText) / 2),
		math.floor(self.font:getHeight() / 2)
	)
	love.graphics.setColor(color255To1(255, 255, 255))
	...
	if self.player.energy_shield then
		love.graphics.print(
			...
			1,
			1,
			math.floor(self.font:getWidth("ES") / 2),
			math.floor(self.font:getHeight() / 2)
		)
	else
		love.graphics.print(
			...
			1,
			1,
			math.floor(self.font:getWidth("HP") / 2),
			math.floor(self.font:getHeight() / 2)
		)
	end
	...
	love.graphics.print(
		...
		1,
		1,
		math.floor(self.font:getWidth(hpText) / 2),
		math.floor(self.font:getHeight() / 2)
	)
	love.graphics.setColor(color255To1(255, 255, 255))
	...
	if self.player.threader then
		love.graphics.rectangle(
			...
			gw / 2 + 4,
			gh - 24,
			48 * (self.player.cycle_timer_2 / self.player.cycle_cooldown_2),
			4
		)
		love.graphics.setColor(color255To1(r - 32, g - 32, b - 32))
		...
		love.graphics.rectangle(
			...
			gw / 2 + 4,
			gh - 32,
			48 * (self.player.cycle_timer_3 / self.player.cycle_cooldown_3),
			4
		)
		love.graphics.setColor(color255To1(r - 32, g - 32, b - 32))
		...
		love.graphics.rectangle(
			...
			gw / 2 + 4,
			gh - 40,
			48 * (self.player.cycle_timer_4 / self.player.cycle_cooldown_4),
			4
		)
		love.graphics.setColor(color255To1(r - 32, g - 32, b - 32))
	...
	if self.player.rampage then
		...
		love.graphics.rectangle(
			...
			gw - 34,
			gh - 16,
			24 * (1 - (self.player.rampage_timer / self.player.rampage_cooldown)),
			4
		)
		love.graphics.setColor(color255To1(r - 32, g - 32, b - 32))
		love.graphics.rectangle("line", gw - 34, gh - 16, 24, 4)
		love.graphics.print(
			self.player.rampage_counter,
			gw - 10 - 12,
			gh - 24,
			0,
			1,
			1,
			math.floor(self.font:getWidth(self.player.rampage_counter) / 2),
			math.floor(self.font:getHeight() / 2)
		)
		love.graphics.setColor(color255To1(255, 255, 255))
	end
	...
	love.graphics.rectangle(
		...
		24
			* (
				self.director.round_timer
				/ (
					self.director.round_duration
					/ (
						self.player.enemy_spawn_rate_multiplier
						* self.director.adaptive_difficulty_enemy_spawn_rate_multiplier
					)
				)
			),
		4
	...
	love.graphics.rectangle(
		...
		24
			* (
				self.director.resource_timer
				/ (
					self.director.resource_duration
					/ (self.player.resource_spawn_rate_multiplier + self.director.first_10_runs_resource_spawn_rate)
				)
			),
		4
	)
	...
	love.graphics.rectangle(
		...
		10,
		26,
		24 * (self.director.attack_timer / (self.director.attack_duration / self.player.attack_spawn_rate_multiplier)),
		4
	)
	love.graphics.setColor(color255To1(r - 32, g - 32, b - 32))
	...
	if self.director.spawned_item_count < maxItems then
		...
		love.graphics.rectangle(
			...
			24
				* (
					self.director.item_timer
					/ (
						self.director.item_duration
						/ (
							self.player.luck_multiplier * self.player.item_spawn_rate_multiplier
							+ self.director.first_10_runs_item_spawn_rate
						)
					)
				),
			4
		...
		love.graphics.print(
			maxItems - self.director.spawned_item_count,
			38,
			35,
			0,
			1,
...
function Stage:finish()
	timer:after(1, function()
		...
		if device == "Fighter" and score >= 10000 then
			unlockAchievement("10K Fighter")
		end
		if device == "Crusader" and score >= 10000 then
			unlockAchievement("10K Crusader")
		end
		if device == "Rogue" and score >= 10000 then
			unlockAchievement("10K Rogue")
		end
		if device == "Bit Hunter" and score >= 10000 then
			unlockAchievement("10K Bit Hunter")
		end
		if device == "Sentinel" and score >= 50000 then
			unlockAchievement("50K Sentinel")
		end
		if device == "Striker" and score >= 50000 then
			unlockAchievement("50K Striker")
		end
		if device == "Nuclear" and score >= 50000 then
			unlockAchievement("50K Nuclear")
		end
		if device == "Cycler" and score >= 50000 then
			unlockAchievement("50K Cycler")
		end
		if device == "Wisp" and score >= 50000 then
			unlockAchievement("50K Wisp")
		end

		if device == "Fighter" and score >= 100000 then
			unlockAchievement("100K Fighter")
		end
		if device == "Crusader" and score >= 100000 then
			unlockAchievement("100K Crusader")
		end
		if device == "Rogue" and score >= 100000 then
			unlockAchievement("100K Rogue")
		end
		if device == "Bit Hunter" and score >= 100000 then
			unlockAchievement("100K Bit Hunter")
		end
		if device == "Sentinel" and score >= 100000 then
			unlockAchievement("100K Sentinel")
		end
		if device == "Striker" and score >= 100000 then
			unlockAchievement("100K Striker")
		end
		if device == "Nuclear" and score >= 100000 then
			unlockAchievement("100K Nuclear")
		end
		if device == "Cycler" and score >= 100000 then
			unlockAchievement("100K Cycler")
		end
		if device == "Wisp" and score >= 100000 then
			unlockAchievement("100K Wisp")
		end

		if device == "Fighter" and score >= 500000 then
			unlockAchievement("500K Fighter")
		end
		if device == "Crusader" and score >= 500000 then
			unlockAchievement("500K Crusader")
		end
		if device == "Rogue" and score >= 500000 then
			unlockAchievement("500K Rogue")
		end
		if device == "Bit Hunter" and score >= 500000 then
			unlockAchievement("500K Bit Hunter")
		end
		if device == "Sentinel" and score >= 500000 then
			unlockAchievement("500K Sentinel")
		end
		if device == "Striker" and score >= 500000 then
			unlockAchievement("500K Striker")
		end
		if device == "Nuclear" and score >= 500000 then
			unlockAchievement("500K Nuclear")
		end
		if device == "Cycler" and score >= 500000 then
			unlockAchievement("500K Cycler")
		end
		if device == "Wisp" and score >= 500000 then
			unlockAchievement("500K Wisp")
		end

		if device == "Fighter" and score >= 1000000 then
			unlockAchievement("1KK Fighter")
		end
		if device == "Crusader" and score >= 1000000 then
			unlockAchievement("1KK Crusader")
		end
		if device == "Rogue" and score >= 1000000 then
			unlockAchievement("1KK Rogue")
		end
		if device == "Bit Hunter" and score >= 1000000 then
			unlockAchievement("1KK Bit Hunter")
		end
		if device == "Sentinel" and score >= 1000000 then
			unlockAchievement("1KK Sentinel")
		end
		if device == "Striker" and score >= 1000000 then
			unlockAchievement("1KK Striker")
		end
		if device == "Nuclear" and score >= 1000000 then
			unlockAchievement("1KK Nuclear")
		end
		if device == "Cycler" and score >= 1000000 then
			unlockAchievement("1KK Cycler")
		end
		if device == "Wisp" and score >= 1000000 then
			unlockAchievement("1KK Wisp")
		end
...
function Stage:pause()
	...
	if self.paused then
		self.paused_object = Paused(self)
	else
		self.paused_object = nil
	end
end
...
function Stage:glitch(x, y, w, h)
	...
	for i = 1, love.math.random(0, 3) do
		self.timer:after(0.1 * i, function()
			self.area:addGameObject("GlitchDisplacement", 0, 0, {
				x = x + random(-w / 2, w / 2),
				y = y + random(-h / 2, h / 2),
				w = random(w / 1.5, 1.5 * w),
				h = random(h / 1.5, 1.5 * h),
				type = "rectangular_block_shift",
			})
		end)
	end
end
