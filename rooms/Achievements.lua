	...
	if input:down("left", 0.05, 0.25) then
		if self.achievement_index > 1 then
			...
			self.timer:tween(
				0.1,
				self,
				{ target_x = gw / 2 + (self.achievement_index - 1) * 80 },
				"in-out-cubic",
				"camera_target"
			)
			playMenuSwitch()
...
function Achievements:drawAchievement(x, y, achievement_name, active)
	...
	local name = ""
	local color = nil
	if achievement_name:find("Reroll") then
		name = "RR"
		color = hp_color
	end
	if achievement_name:find("Escape") then
		name = "WIN"
		color = skill_point_color
	end
	if achievement_name:find("10K") then
		name = "10K"
		color = ammo_color
	end
	if achievement_name:find("50K") then
		name = "50K"
		color = ammo_color
	end
	if achievement_name:find("100K") then
		name = "100K"
		color = boost_color
	end
	if achievement_name:find("500K") then
		name = "500K"
		color = hp_color
	end
	if achievement_name:find("1KK") then
		name = "1KK"
		color = skill_point_color
	end
	...
	love.graphics.print(
		...
		2,
		2,
		math.floor(self.font:getWidth(name) / 2),
		math.floor(self.font:getHeight() / 2)
	)

	y = y + 28
	local device = ""
	if achievement_name:find("Fighter") then
		device = "Fighter"
	end
	if achievement_name:find("Crusader") then
		device = "Crusader"
	end
	if achievement_name:find("Rogue") then
		device = "Rogue"
	end
	if achievement_name:find("Bit Hunter") then
		device = "Bit Hunter"
	end
	if achievement_name:find("Sentinel") then
		device = "Sentinel"
	end
	if achievement_name:find("Striker") then
		device = "Striker"
	end
	if achievement_name:find("Nuclear") then
		device = "Nuclear"
	end
	if achievement_name:find("Cycler") then
		device = "Cycler"
	end
	if achievement_name:find("Wisp") then
		device = "Wisp"
	end
	if device == "" then
		goto continue
	...
	for _, vertice_group in ipairs(self.device_vertices[device].vertice_groups) do
		local points = fn.map(vertice_group, function(k, v)
			if k % 2 == 1 then
				return x + v + random(-1, 1)
			else
				return y + v + random(-1, 1)
			end
		end)
	...
	love.graphics.pop()
	y = y - self.y_offsets[device]
	-- workaround for stylua, don't remove
	...
	elseif achievement_name == "Escape" then
		...
		love.graphics.print(
			...
			1,
			1,
			math.floor(self.font:getWidth("escape") / 2),
			math.floor(self.font:getHeight() / 2)
		)
		...
		love.graphics.rectangle(
			"fill",
			x + 16 - math.floor(self.font:getWidth("w") / 2),
			y - math.floor(self.font:getHeight() / 2),
			self.font:getWidth("w"),
			self.font:getHeight()
		)
	...
	if active then
		...
		if achievements[achievement_name] then
			...
			love.graphics.print(
				...
				1,
				1,
				math.floor(self.font:getWidth("UNLOCKED") / 2),
				math.floor(self.font:getHeight() / 2)
			)
		else
			...
			love.graphics.print(
				...
				1,
				1,
				math.floor(self.font:getWidth("LOCKED") / 2),
				math.floor(self.font:getHeight() / 2)
			)
		end
		love.graphics.setColor(color255To1(default_color))
		...
		y = y + 16
		love.graphics.print(
			achievement_descriptions[achievement_name],
			x,
			y,
			0,
			1,
			1,
			math.floor(self.font:getWidth(achievement_descriptions[achievement_name]) / 2),
			math.floor(self.font:getHeight() / 2)
		)
	end
end
