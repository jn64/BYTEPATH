...
function Director:new(stage, starting_difficulty)
	...
	for i = 40, 2048 do
		self.difficulty_to_points[i] = 160 + 2 * i
	end
	...
	for i = 21, 1024 do
		self.enemy_spawn_chances[i] = chanceList(
			...
			{ "Sapper", love.math.random(2, 12) },
			{ "Roller", love.math.random(2, 12) },
			{ "Seeker", love.math.random(2, 12) },
			{ "Waver", love.math.random(2, 12) }, --[[{'Rotator', love.math.random(2, 12)},]]
			{ "Tanker", love.math.random(2, 12) },
			{ "Triad", love.math.random(2, 6) },
			{ "Trailer", love.math.random(2, 12) },
			{ "Reflecteer", love.math.random(2, 12) },
			{ "Orbitter", love.math.random(2, 12) },
			{ "Glitcher", love.math.random(2, 6) }
		)
	end

	local first_10_runs_sp_spawn_chance_multiplier = 1
	if run <= 15 then
		first_10_runs_sp_spawn_chance_multiplier = 1 + (15 - run) / 5
	end
	self.resource_spawn_chances = chanceList(
		...
		{ "SkillPoint", 92 * self.stage.player.sp_spawn_chance_multiplier * first_10_runs_sp_spawn_chance_multiplier }
	)

	self.first_10_runs_resource_spawn_rate = 0
	if run <= 15 then
		self.first_10_runs_resource_spawn_rate = (15 - run) / 15
	end

	self.first_10_runs_item_spawn_rate = 0
	if run <= 15 then
		self.first_10_runs_item_spawn_rate = (15 - run) / 15
	end
...
function Director:update(dt)
	...
	self.adaptive_difficulty_enemy_spawn_rate_multiplier =
		math.max(3 * (self.stage.enemies_killed / self.stage.enemies_created) ^ 4, 1)
	...
	if
		self.round_timer
		> self.round_duration
			/ (self.stage.player.enemy_spawn_rate_multiplier * self.adaptive_difficulty_enemy_spawn_rate_multiplier)
	then
		self.round_timer = 0
	...
	if self.difficulty <= 40 then
		self.enemy_hp_multiplier = 1
	else
		self.enemy_hp_multiplier = 1 + (self.difficulty * self.difficulty) / 2400
	end
	...
	if
		...
		> self.resource_duration
			/ (self.stage.player.resource_spawn_rate_multiplier + self.first_10_runs_resource_spawn_rate)
	then
		self.resource_timer = 0
			...
			elseif resource == "SkillPoint" then
				if self.stage.player.chances.spawn_double_sp_chance:next() then
					...
					self.stage.area:addGameObject(
						...
						self.stage.player.x,
						self.stage.player.y,
						{ text = "Double SP!", color = skill_point_color }
					)
				end
			end
		end
	...
	if self.attack_timer > self.attack_duration / self.stage.player.attack_spawn_rate_multiplier then
		self.attack_timer = 0
		if self.stage.player.only_spawn_boost then
			self.stage.area:addGameObject("Boost")
		else
			self.stage.area:addGameObject("Attack")
		end
	end
	...
	if
		self.item_timer
			> self.item_duration / (self.stage.player.luck_multiplier * self.stage.player.item_spawn_rate_multiplier + self.first_10_runs_item_spawn_rate)
		and self.spawned_item_count <= math.floor(5 * self.stage.player.item_spawn_rate_multiplier)
	then
		self.item_timer = 0
		self.stage.area:addGameObject("Item")
