...
function love.update(dt)
	...
	if fps > 10 and pre_disable_expensive_shaders then
		pre_disable_expensive_shaders = false
	end
...
function love.keypressed(key)
	if current_room and current_room.keypressed then
...
function frameStop(duration, object_types)
	if current_room then
		current_room.area:frameStop(duration, object_types)
	end
end
