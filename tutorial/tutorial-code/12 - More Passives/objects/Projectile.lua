...
function Projectile:new(area, x, y, opts)
	...
	if self.attack == "Spin" then
		...
		self.timer:after(random(2.4, 3.2), function()
			self:die()
		end)
		self.timer:every(0.05, function()
			self.area:addGameObject(
				...
				self.x,
				self.y,
				{ r = Vector(self.collider:getLinearVelocity()):angle(), color = self.color, s = self.s }
			)
		end)
	end
...
function Projectile:die()
	...
	if self.attack == "Spread" then
		self.area:addGameObject(
			...
			self.x,
			self.y,
			{ color = table.random(all_colors), w = 3 * self.s }
		)
	else
		self.area:addGameObject(
			"ProjectileDeathEffect",
