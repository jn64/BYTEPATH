...
local function clamp(var, a, b)
	return (var < a) and a or (var > b and b or var)
end
local function isTrue(_, value)
	return value and true
end
local function iNot(value)
	return not value
end
...
function _.sortBy(t, transform, comp)
	...
	t_sort(_t, function(a, b)
		return comp(a.transform, b.transform)
	end)
...
function _.sampleProb(array, prob, seed)
	...
	return _.select(array, function(_, v)
		return random() < prob
	end)
end
...
function _.find(array, value, from)
	for i = from or 1, #array do
		if _.isEqual(array[i], value) then
			return i
		end
	end
end
...
function _.flatten(array, shallow)
	...
	for key, value in pairs(array) do
		if _.isTable(value) then
			...
			_.each(new_flattened, function(_, item)
				_flat[#_flat + 1] = item
			end)
		else
			_flat[#_flat + 1] = value
...
function _.symmetricDifference(array, array2)
	return _.difference(_.union(array, array2), _.intersection(array, array2))
end
...
function _.bindAll(obj, ...)
	...
	for __, methodName in ipairs(methodNames) do
		...
		if method then
			obj[methodName] = _.bind(method, obj)
		end
	end
	return obj
end
...
function _.overEvery(...)
	local f = _.over(...)
	return function(...)
		return _.reduce(f(...), function(state, v)
			return state and v
		end)
	end
end
...
function _.values(obj)
	local _oValues = {}
	_.each(obj, function(_, value)
		_oValues[#_oValues + 1] = value
	end)
	return _oValues
end
...
function _.isEqual(objA, objB, useMt)
	...
	if _.size(objA) ~= _.size(objB) then
		return false
	end

	for i, v1 in pairs(objA) do
		local v2 = objB[i]
		if _.isNil(v2) or not _.isEqual(v1, v2, useMt) then
			return false
		end
	end
...
function _.isCallable(obj)
	return (_.isFunction(obj) or (_.isTable(obj) and getmetatable(obj) and getmetatable(obj).__call ~= nil) or false)
end
...
do
	...
	setmetatable(__, {
		__call = function(self, v)
			return new(v)
		end, -- Calls returns to instantiation
		__index = function(t, key, ...)
			return f[key]
		end, -- Redirects to the wrapper
	})
