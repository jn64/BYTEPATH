...
context("Table functions specs", function()
	...
	context("reduce", function()
		...
		test("initial state defaults to the first value when not given", function()
			assert_equal(
				_.reduce({ "a", "b", "c" }, function(memo, v)
					return memo .. v
				end),
				"abc"
			)
		...
		test("supports arrays of booleans", function()
			assert_equal(
				_.reduce({ true, false, true, true }, function(memo, v)
					return memo and v
				end),
				false
			)
			assert_equal(
				_.reduce({ true, true, true }, function(memo, v)
					return memo and v
				end),
				true
			)
			assert_equal(
				_.reduce({ false, false, false }, function(memo, v)
					return memo and v
				end),
				false
			)
			assert_equal(
				_.reduce({ false, false, true }, function(memo, v)
					return memo or v
				end),
				true
			)
	...
	context("reduceby", function()
		test("folds a collection (left to right) for specific values", function()
			...
			assert_equal(
				_.reduceby({ 1, 2, 3, 4 }, function(memo, v)
					return memo + v
				end, 0, even),
				6
			)
	...
	context("reduceRight", function()
		...
		test("initial state defaults to the first value when not given", function()
			assert_equal(
				_.reduceRight({ "a", "b", "c" }, function(memo, v)
					return memo .. v
				end),
				"cba"
			)
	...
	context("mapReduce", function()
		test("folds a collection (left to right) saving intermediate states", function()
			assert_true(_.isEqual(
				_.mapReduce({ 1, 2, 4, 16 }, function(memo, v)
					return memo * v
				end, 0),
				{ 0, 0, 0, 0 }
			))
	...
	context("mapReduceRight", function()
		test("folds a collection (right to left) saving intermediate states", function()
			assert_true(_.isEqual(
				_.mapReduceRight({ 1, 2, 4, 16 }, function(memo, v)
					return memo / v
				end, 256),
				{ 16, 4, 2, 2 }
			))
	...
	context("detect", function()
		...
		test("given an iterator, return the key of the first value passing a truth test", function()
			assert_equal(
				_.detect({ "a", "B", "c" }, function(array_value)
					return (array_value:upper() == array_value)
				end),
				2
			)
	...
	context("invoke", function()
		test("calls an iterator over each object, passing it as a first arg", function()
			assert_true(_.isEqual(_.invoke({ "a", "bea", "cdhza" }, string.len), { 1, 3, 5 }))

			assert_true(
				_.isEqual(
					_.invoke({ { 2, 3, 2 }, { 13, 8, 10 }, { 0, -5 } }, _.sort),
					{ { 2, 2, 3 }, { 8, 10, 13 }, { -5, 0 } }
				)
			)
		...
		test("given a string, calls the matching object property the same way", function()
			...
			assert_true(_.isEqual(_.invoke({ a, b, c, d }, "call"), { a, b, c, d }))
		end)
	end)
	...
	context("pluck", function()
		test("fetches a property value in a collection of objects", function()
			...
			assert_true(_.isEqual(_.pluck(peoples, "age"), { 23, 17, 15, 33 }))
			assert_true(_.isEqual(_.pluck(peoples, "name"), { "John", "Peter", "Steve" }))
		end)
	end)
	...
	context("groupBy", function()
		test("splits a collection into subsets of items behaving the same", function()
			...
			assert_true(_.isEqual(
				_.groupBy({ "one", "two", "three", "four", "five" }, function(i, value)
					return value:len()
				end),
		...
		test("can takes extra-args", function()
			assert_true(_.isEqual(
				_.groupBy({ 3, 9, 10, 12, 15 }, function(k, v, x)
					return v % x == 0
				end, 2),
				{ [false] = { 3, 9, 15 }, [true] = { 10, 12 } }
			))
			assert_true(_.isEqual(
				_.groupBy({ 3, 9, 10, 12, 15 }, function(k, v, x)
					return v % x == 0
				end, 3),
				{ [false] = { 10 }, [true] = { 3, 9, 12, 15 } }
			))
		end)
	end)
	...
	context("countBy", function()
		test("splits a collection in subsets and counts items inside", function()
			...
			assert_true(_.isEqual(
				_.countBy({ 0, "a", true, false, nil, b, 0.5 }, function(i, value)
					return type(value)
				end),
				{ number = 2, string = 1, boolean = 2 }
			))

			assert_true(_.isEqual(
				_.countBy({ "one", "two", "three", "four", "five" }, function(i, value)
					return value:len()
				end),
				{ [3] = 2, [4] = 2, [5] = 1 }
			))
