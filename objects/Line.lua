...
function Line:update(dt)
	local cx, cy =
		camera:getCameraCoords((self.node_1.x + self.node_2.x) / 2, (self.node_1.y + self.node_2.y) / 2, 0, 0, gw, gh)
	...
	if self.node_1.bought and self.node_2.bought then
		self.selected = true
	else
		self.selected = false
	end
...
function Line:draw()
	...
	for i = 1, #self.points do
		...
		if next_point then
			love.graphics.line(point.x, point.y, next_point.x, next_point.y)
		end
	end
