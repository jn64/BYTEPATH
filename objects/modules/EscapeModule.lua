	...
	then
		...
		for i = 1, 8 do
			if found_keys[i] == 1 then
				self.console:addLine(
					0.06 + i * 0.02,
					"    key:" .. keys[i].address .. "                         [  <FOUND>  ]"
				)
			else
				self.console:addLine(
					0.06 + i * 0.02,
					"    ;key:" .. keys[i].address .. ", found in difficulty @" .. i * 5 .. "+#"
				)
			end
		end
		...
		timer:after(46, function()
			...
			for i, class in ipairs(classes) do
				self.console:addLine(2.02 + 0.02 * i, "[   <OK>   ] removed class @" .. class .. "#")
			end
			local n = #classes
			self.console:addLine(
				2.04 + 0.04 + n * 0.02,
			...
			for i, node_index in ipairs(bought_node_indexes) do
				self.console:addLine(t + 2.02 + 0.02 * i, "[   <OK>   ] removed node @(" .. node_index .. ")#")
			end
		...
		self.console.timer:after(0.4, function()
			self.active = false
		end)
	end
end
...
function EscapeModule:draw() end
