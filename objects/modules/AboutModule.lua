...
function AboutModule:update(dt)
	...
	if input:pressed("return") then
		...
		for i = 1, #command do
			...
			self.console.timer:after(0.04 + 0.025 * i, function()
				love.event.push("textinput", c)
			end)
		end
		self.console.timer:after(0.04 + 0.025 * (#command + 1) + 0.25, function()
			if self.console.input_line then
				self.console.input_line:enter()
			end
		end)
	end
end
