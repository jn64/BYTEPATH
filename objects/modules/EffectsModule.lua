...
function EffectsModule:update(dt)
	...
	if input:pressed("left") then
		if self.selection_index == 1 then
			self:changeValue("distortion", -1)
		elseif self.selection_index == 2 then
			self:changeValue("screen_shake", -1)
		elseif self.selection_index == 3 then
			self:changeValue("glitch", -1)
		end
		self:setSelectionWidths()
	end
...
function EffectsModule:setSelectionWidths()
	self.selection_widths = {
		self.console.font:getWidth("distortion: " .. tostring(distortion)),
		self.console.font:getWidth("screen shake: " .. tostring(screen_shake)),
		self.console.font:getWidth("glitch: " .. tostring(glitch)),
	}
end
