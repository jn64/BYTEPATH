...
function ClearModule:new(console, y)
	...
	self.console.timer:after(0.10, function()
		self.active = true
	end)
...
function ClearModule:update(dt)
			...
			if not self.confirm_selecting then
				...
				elseif self.selection_index == 2 then
					...
					self.console:addLine(
						0.02,
						"@WARNING!!!# This will erase all bought passives on your passive skill tree."
					)
					self.console:addLine(0.04, "You will @NOT# be given any skill points back.")
					self.console:addLine(0.06, "Are you sure you want to proceed?")
					self.console:addLine(0.08, "    YES")
	...
	elseif self.active and self.confirm_selecting then
		if input:pressed("up") then
			...
			if self.confirm_selection_index < 1 then
				self.confirm_selection_index = #self.confirm_selection_widths
			end
		end

		if input:pressed("down") then
			self.confirm_selection_index = self.confirm_selection_index + 1
			if self.confirm_selection_index > #self.confirm_selection_widths then
				self.confirm_selection_index = 1
			end
		end
		...
		if input:pressed("return") then
			if self.confirm_selection_index == 2 then
				...
				if self.selection_index == 1 then
					if #classes >= 9 then
						unlockAchievement("Reroll Classes")
					end
					self.active = false
					self.console:addLine(0.02, ";~RESETTING CLASSES!~,")
					for i, class in ipairs(classes) do
						self.console:addLine(0.02 + 0.02 * i, "[   <OK>   ] removed class @" .. class .. "#")
					end
					local n = #classes
					self.console:addLine(
						0.04 + 0.04 + n * 0.02,
						"[  ;WAIT,  ] removing core targets",
						0.8,
						{ { ";WAIT,", " <OK> " }, { "removing", "removed" } }
					)
					...
					save()
				elseif self.selection_index == 2 then
					if #bought_node_indexes >= 80 then
						unlockAchievement("Reroll Skills")
					end
					self.active = false
					self.console:addLine(0.02, ";~RESETTING SKILL TREE!~,")
					for i, node_index in ipairs(bought_node_indexes) do
						self.console:addLine(0.02 + 0.02 * i, "[   <OK>   ] removed node @(" .. node_index .. ")#")
					end
					local n = #bought_node_indexes
					self.console:addLine(
						0.02 + 0.04 + n * 0.02,
						"[  ;WAIT,  ] unlinking paths",
						1.4,
						{ { ";WAIT,", " <OK> " }, { "unlinking", "unlinked" } }
					)
					self.console:addLine(
						0.04 + 0.04 + n * 0.02,
						"[  ;WAIT,  ] removing core targets",
					...
					self.console:addLine(
						0.06 + 0.04 + n * 0.02,
						"[  ;WAIT,  ] unmounting ("
							.. self.console:getRandomArchWord()
							.. ") ["
							.. self.console:getRandomArchWord()
							.. "]",
						0.4,
						{ { ";WAIT,", " <OK> " }, { "unmounting", "unmounted" } }
					)
					...
					save()
				elseif self.selection_index == 3 then
					if #classes >= 9 and #bought_node_indexes >= 80 then
						unlockAchievement("Reroll Build")
					end
					if #classes >= 9 then
						unlockAchievement("Reroll Classes")
					end
					...
					for i, node_index in ipairs(bought_node_indexes) do
						self.console:addLine(t + 0.02 + 0.02 * i, "[   <OK>   ] removed node @(" .. node_index .. ")#")
					end
					local n = #bought_node_indexes
					self.console:addLine(
						t + 0.02 + 0.04 + n * 0.02,
					...
					self.console:addLine(
						t + 3.80,
						"[   <OK>   ] removed target (" .. self.console:getRandomArchWord() .. ")"
					)
					self.console:addLine(
						t + 3.81,
						"[   <OK>   ] removed target (" .. self.console:getRandomArchWord() .. ")"
					...
					self.console:addLine(
						t + 4.81,
						"[  ;WAIT,  ] deallocating system ("
							.. self.console:getRandomArchWord()
							.. ") ["
							.. self.console:getRandomArchWord()
							.. "]",
						1.1,
						{ { ";WAIT,", " <OK> " }, { "deallocating", "deallocated" } }
					)
					self.console:addLine(
						t + 5.2,
						"[  ;WAIT,  ] stopping (" .. self.console:getRandomArchWord() .. ") devices",
						0.4,
						{ { ";WAIT,", " <OK> " }, { "stopping", "stopped" } }
					)
					self.console:addLine(
						t + 5.60,
						"[  ;WAIT,  ] stopping virtual console",
						1.2,
						{ { ";WAIT,", " <OK> " }, { "stopping", "stopped" } }
					)
					self.console:addLine(
						t + 5.61,
						"[  ;WAIT,  ] unmounting (" .. self.console:getRandomArchWord() .. ") message queue file system",
						1.4,
						{ { ";WAIT,", " <OK> " }, { "unmounting", "unmounted" } }
					)
					self.console:addLine(
					...
					self.console:addLine(
						...
						"[  ;WAIT,  ] shutting down...",
						2.0,
						{ { ";WAIT,", " <OK> " }, { "shutting", "shut" } }
					)
					self.console:addLine(t + 14.20, "[   <OK>   ] stopped message system")
					self.console:addLine(
						t + 14.22,
						"[  ;WAIT,  ] stopping bioware abstraction layer",
						1.4,
						{ { ";WAIT,", " <OK> " }, { "stopping", "stopped" } }
					)
					...
					self.console:addLine(
						t + 14.26,
						"[  ;WAIT,  ] sending [" .. self.console:getRandomArchWord() .. "] to processes",
						0.9,
						{ { ";WAIT,", " <OK> " }, { "sending", "sent" } }
					)
					self.console:addLine(
						t + 14.28,
						"[  ;WAIT,  ] sending [" .. self.console:getRandomArchWord() .. "] to processes",
...
function ClearModule:draw()
	if self.active and not self.confirm_selecting then
		...
		if self.selection_index == 2 then
			r, g, b = unpack(skill_point_color)
		end
		if self.selection_index == 3 then
			r, g, b = unpack(hp_color)
		end
		...
		love.graphics.rectangle(
			"fill",
			8 + x_offset - 2,
			self.y + self.selection_index * 12 + 17 + 12,
			width + 4,
	...
	elseif self.active and self.confirm_selecting then
		...
		love.graphics.rectangle(
			"fill",
			8 + x_offset - 2,
			self.y + self.confirm_selection_index * 12 + 8 * 12 + 17,
			width + 4,
			self.console.font:getHeight()
		)
		love.graphics.setColor(color255To1(255, 255, 255, 255))
