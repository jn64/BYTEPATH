...
function DeviceModule:new(console, y)
	...
	self.console:addLine(
		0.12,
		"~ ;SP: " .. tostring(skill_points) .. ", / $CURRENT DEVICE: " .. string.upper(device) .. "%"
	)
...
function DeviceModule:update(dt)
	...
	if input:pressed("left") then
		...
		if self.device_index == 0 then
			self.device_index = #self.devices
		end
	...
	if input:pressed("right") then
		self.device_index = self.device_index + 1
		if self.device_index == #self.devices + 1 then
			self.device_index = 1
		end
		local current_device = self.devices[self.device_index]
		if fn.any(unlocked_devices, current_device) then
	...
	if input:pressed("return") then
		...
		if not fn.any(unlocked_devices, current_device) then
			if skill_points >= self.device_costs[current_device] then
				self.device_line:replace(
					tostring(skill_points),
					tostring(skill_points - self.device_costs[current_device])
				)
			...
			self.console.timer:after(0.06, function()
				self.console:bytepathMain()
			end)
		end
	end
	...
	if input:pressed("escape") then
		self.active = false
		self.console:addLine(0.02, "")
		self.console.timer:after(0.04, function()
			self.console:bytepathMain()
		end)
	end
end
...
function DeviceModule:draw()
	...
	for _, vertice_group in ipairs(self.device_vertices[self.devices[self.device_index]].vertice_groups) do
		local points = fn.map(vertice_group, function(k, v)
			if k % 2 == 1 then
				return self.x - self.w / 2 + 10 + v + random(-1, 1)
			else
				return self.y - 2 + self.device_y_offsets[device] + v + random(-1, 1)
			end
		end)
	...
	if fn.any(unlocked_devices, device) then
		love.graphics.print(
			"UNLOCKED",
			self.x - self.w / 2 + 10,
			self.y + 23 + self.device_y_offsets[device] / 2,
			0,
			1.01,
			1.01,
			self.font:getWidth("UNLOCKED") / 2,
			self.font:getHeight() / 2
		)
	else
		love.graphics.setColor(color255To1(skill_point_color))
		love.graphics.print(
			"LOCKED - " .. self.device_costs[device] .. "SP",
			self.x - self.w / 2 + 10,
			self.y + 23 + self.device_y_offsets[device] / 2,
			0,
			1.01,
			1.01,
			self.font:getWidth("LOCKED - " .. self.device_costs[device] .. "SP") / 2,
			self.font:getHeight() / 2
		)
		love.graphics.setColor(color255To1(default_color))
	end
