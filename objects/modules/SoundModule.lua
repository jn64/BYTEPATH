...
function SoundModule:update(dt)
	...
	if input:pressed("left") then
		if self.selection_index == 1 then
			self:changeValue("main_volume", -1)
		elseif self.selection_index == 2 then
			self:changeValue("sfx_volume", -1)
		elseif self.selection_index == 3 then
			self:changeValue("music_volume", -1)
...
function SoundModule:draw()
	...
	love.graphics.rectangle(
		"fill",
		8 + x_offset - 2,
		self.y + self.selection_index * 12 + 17,
		width + 4,
...
function SoundModule:setSelectionWidths()
	self.selection_widths = {
		self.console.font:getWidth("main volume: " .. tostring(main_volume)),
		self.console.font:getWidth("sfx volume: " .. tostring(sfx_volume)),
		self.console.font:getWidth("music volume: " .. tostring(music_volume)),
		self.console.font:getWidth("muted: " .. tostring(muted)),
	}
end
