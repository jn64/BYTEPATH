...
function LaserLine:new(area, x, y, opts)
	...
	self.timer:tween(0.2, self, { line_width = 1, s = 8, b = 16 }, "in-out-cubic", function()
		self.dead = true
	end)
	self.timer:after(0.1, function()
		self.timer:tween(0.1, self, { side_line_width = 0 }, "in-out-cubic")
	end)
end
...
function LaserLine:draw()
	...
	local x1u, y1u =
		self.x1 + self.s * math.cos(self.angle - math.pi / 2), self.y1 + self.s * math.sin(self.angle - math.pi / 2)
	local x1d, y1d =
		self.x1 + self.s * math.cos(self.angle + math.pi / 2), self.y1 + self.s * math.sin(self.angle + math.pi / 2)
	local x2u, y2u =
		self.x2 + self.s * math.cos(self.angle - math.pi / 2), self.y2 + self.s * math.sin(self.angle - math.pi / 2)
	local x2d, y2d =
		self.x2 + self.s * math.cos(self.angle + math.pi / 2), self.y2 + self.s * math.sin(self.angle + math.pi / 2)
