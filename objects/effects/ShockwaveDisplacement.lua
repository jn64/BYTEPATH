...
function ShockwaveDisplacement:new(area, x, y, opts)
	...
	self.timer:tween(
		0.5,
		self,
		{ sx = 0.75 * (opts.wm or 1), sy = 0.75 * (opts.wm or 1), alpha = 0 },
		"linear",
		function()
			self.dead = true
		end
...
function ShockwaveDisplacement:draw()
	...
	love.graphics.draw(
		assets.shockwave_displacement,
		self.x,
		self.y,
		0,
		self.sx,
		self.sy,
		assets.shockwave_displacement:getWidth() / 2,
		assets.shockwave_displacement:getHeight() / 2
	)
