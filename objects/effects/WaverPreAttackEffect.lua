...
function WaverPreAttackEffect:new(area, x, y, opts)
	...
	self.s = 1
	self.alpha = 255
	self.timer:tween(0.35, self, { alpha = 0, s = 4 }, "in-out-cubic", function()
		self.dead = true
	end)
end
...
function WaverPreAttackEffect:update(dt)
	...
	if self.waver and not self.waver.dead then
		self.x, self.y = self.waver.x, self.waver.y
	end
end

function WaverPreAttackEffect:draw()
	if not self.waver or self.waver.dead then
		return
	end
