...
function PreAttackEffect:update(dt)
	...
	if self.shooter and not self.shooter.dead then
		self.x, self.y =
			self.shooter.x + 1.4 * self.shooter.w * math.cos(self.shooter.r),
			self.shooter.y + 1.4 * self.shooter.w * math.sin(self.shooter.r)
	end

	if self.seeker and not self.seeker.dead then
		self.x, self.y =
			self.seeker.x + 1.4 * self.seeker.w * math.cos(self.seeker.r - math.pi),
			self.seeker.y + 1.4 * self.seeker.w * math.sin(self.seeker.r - math.pi)
	end
end
