...
function LightningLine:new(area, x, y, opts)
	...
	self.alpha = 255
	self.timer:tween(self.duration, self, { alpha = 0 }, "in-out-cubic", function()
		self.dead = true
	end)
end
...
function LightningLine:generate()
	...
	for j = 1, self.generations do
		for i = #lines, 1, -1 do
			...
			local pnx, pny =
				Vector.perpendicular(Vector.normalize(end_point_x - start_point_x, end_point_y - start_point_y))
