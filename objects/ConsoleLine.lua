	...
	if self.duration then
		self.timer:after(self.duration, function()
			for _, swap in ipairs(self.swaps) do
				self.text = self.text:gsub(swap[1], swap[2])
			end
			self:setCharacters()
			self:setGlitch()
		end)
...
function ConsoleLine:setCharacters()
	...
	for i, _ in ipairs(self.characters) do
		self.timer:after(0.0025 * i, function()
			if self.characters and self.characters[i] then
				self.characters[i].visible = true
			end
			if self.base_characters and self.base_characters[i] then
				self.base_characters[i].visible = true
			end
		end)
	end
end
...
function ConsoleLine:draw()
	...
	local arch_font = fonts.Arch_16
	for i = 1, #self.characters do
		if not self.characters[i].visible then
			return
		end
		...
		if not self.characters[i].marker then
			...
			if self.background_colors[i] then
				...
				love.graphics.rectangle(
					"fill",
					self.x + self.characters[i].width,
					self.y + math.floor(normal_font:getHeight() / 2),
					normal_font:getWidth(self.characters[i].c),
			...
			if self.characters[i].language == "normal" then
				...
				love.graphics.print(self.characters[i].c, self.x + self.characters[i].width, self.y, 0, 1, 1, 0, 0)
			end
		end
	end
