...
function RollerPool:new(area, x, y, opts)
	...
	self.timer:after(6, function()
		self.timer:tween(random(0.5, 1), self, { w = 0, h = 0, r = 0 }, "in-out-cubic", function()
			self.dead = true
		end)
	end)
end
...
function RollerPool:update(dt)
	...
	if
		player.x >= self.x - self.w / 2
		and player.x <= self.x + self.w / 2
		and player.y >= self.y - self.h / 2
		and player.y <= self.y + self.h / 2
	then
		player.inside_roller_pool = true
	end
end
