	...
	self.timer:every(random(4, 6), function()
		self.timer:every(0.05, function()
			...
			if direction == "left" then
				x, y = self.x - self.w - 8, random(self.y - self.h / 2, self.y + self.h / 2)
			elseif direction == "right" then
				x, y = self.x + self.w + 8, random(self.y - self.h / 2, self.y + self.h / 2)
			elseif direction == "up" then
				x, y = random(self.x - self.w / 2, self.x + self.w / 2), self.y - self.h - 8
			elseif direction == "down" then
				x, y = random(self.x - self.w / 2, self.y + self.w / 2), self.y + self.h + 8
			end
			local angle = (direction == "left" and math.pi)
				or (direction == "right" and 0)
				or (direction == "up" and -math.pi / 2)
				or (direction == "down" and math.pi / 2)
...
function Glitcher:draw()
	...
	love.graphics.rectangle(
		"fill",
		self.x - self.w,
		self.y - self.h + (2 * self.h - 2 * self.h * (self.hp / self.max_hp)),
		2 * self.w,
		2 * self.h * (self.hp / self.max_hp)
	)
...
function Glitcher:hit(damage)
	...
	if self.hp <= 0 then
		...
		self.hit_flash = true
		self.timer:after("hit_flash", 0.2, function()
