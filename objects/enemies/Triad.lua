			...
			self.timer:after(0.5, function()
				self.timer:every(0.02, function()
					...
					self.area:addGameObject(
						"EnemyProjectile",
						self.x + self.h * math.cos(-math.pi / 2 + angle),
						self.y + self.h * math.sin(-math.pi / 2 + angle),
						{ r = -math.pi / 2 + angle }
					)
					self.area:addGameObject(
						"EnemyProjectile",
						self.x + self.h * math.cos(math.pi / 4 + angle),
						self.y + self.h * math.sin(math.pi / 4 + angle),
						{ r = math.pi / 4 + angle }
					)
					self.area:addGameObject(
						"EnemyProjectile",
						self.x + self.h * math.cos(3 * math.pi / 4 + angle),
						self.y + self.h * math.sin(3 * math.pi / 4 + angle),
						{ r = 3 * math.pi / 4 + angle }
					)
				end, 50)
				self.timer:after(1, function()
					...
					self.timer:tween(
						0.5,
						self,
						{ angv = random(-0.5, 0.5), v = -self.direction * random(10, 20) },
						"in-out-cubic"
					)
				end)
			end)
...
function Triad:update(dt)
	...
	if self.immune then
		self.shape:setRotation(self.angv)
	else
		self.shape:rotate(self.angv * dt)
	end
