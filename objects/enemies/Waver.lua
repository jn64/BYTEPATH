...
function Waver:new(area, x, y, opts)
	...
	self.timer:every(random(2.2, 2.6), function()
		self.timer:every(0.25, function()
			self.area:addGameObject("WaverPreAttackEffect", self.x, self.y, { waver = self })
		end, 3)
			...
			for i = 1, 4 do
				self.timer:after((i - 1) * 0.075, function()
					self.area:addGameObject(
						"EnemyProjectile",
						self.x + 1.4 * self.w * math.cos(self.r),
						self.y + 1.4 * self.w * math.sin(self.r),
						{ r = self.r, v = random(140, 160) }
					)
					self.area:addGameObject(
						"EnemyProjectile",
						self.x + 1.4 * self.w * math.cos(self.r - math.pi),
						self.y + 1.4 * self.w * math.sin(self.r - math.pi),
