...
function ConsoleInputLine:update(dt)
	...
	if (self.console.input_line and self.console.input_line.id ~= self.id) or not self.console.input_line then
		return
	end
	...
	if input:pressed("up") and not self.console.bytepath_main_active then
		if #command_history > 0 then
			...
			if command_history_index < 1 then
			...
			for i = 1, 8 do
				...
				if key_string == kav then
					...
					self.console:addLine(
						0.04,
						"[  ;WAIT,  ] verifying key " .. kav,
						4.4,
						{ { ";WAIT,", " <OK> " }, { "verifying", "verified" } }
					)
					local r = love.math.random(4, 6)
					local words = {}
					for i = 1, r do
						words[i] = self.console:getRandomArchWord()
					end
					for i = 1, r do
						self.console:addLine(
							0.04 + 0.02 * i,
							"[  ;WAIT,  ] connecting to node [" .. words[i] .. "]",
							random(1.2, 2.2),
							{ { ";WAIT,", " <OK> " }, { "connecting", "connected" } }
						)
					end
					self.console:addLine(
						2.46,
						"[  ;WAIT,  ] verifying node integrity",
						1.6,
						{ { ";WAIT,", " <OK> " }, { "verifying", "verified" } }
					)
					for i = 1, r do
						self.console:addLine(
							2.5 + 0.02 * i,
							"[  ;WAIT,  ] verifying node data [" .. words[i] .. "]",
							random(0.8, 1.4),
							{ { ";WAIT,", " <OK> " }, { "verifying", "verified" } }
						)
					end
...
function ConsoleInputLine:draw()
	...
	for i = 1, #self.characters do
		...
		if i > 1 then
			for j = 1, i - 1 do
				if
					...
					and self.characters[j].c ~= "%"
				then
					if self.characters[j].language == "normal" then
						width = width + normal_font:getWidth(self.characters[j].c)
					elseif self.characters[j].language == "arch" then
						width = width + arch_font:getWidth(self.characters[j].c)
					end
				end
			end
		end
		if self.characters[i].language == "normal" then
			love.graphics.setFont(normal_font)
		elseif self.characters[i].language == "arch" then
			love.graphics.setFont(arch_font)
		end
		...
		if self.characters[i].c == "$" then
			love.graphics.setColor(color255To1(boost_color))
		end
		if self.characters[i].c == "%" then
			love.graphics.setColor(color255To1(default_color))
		end
		...
		then
			if self.characters[i].language == "normal" then
				love.graphics.print(
					self.characters[i].c,
					self.x + width,
					self.y + math.floor(normal_font:getHeight() / 2),
					0,
					1,
					1,
					0,
				...
				love.graphics.print(self.characters[i].c, self.x + width, self.y, 0, 1, 1, 0, 0)
			end
		end
	end
	local width = 0
	...
	if self.console.input_line and self.console.input_line.id == self.id then
		...
		if self.cursor_show then
			love.graphics.rectangle(
				"fill",
				self.x + width,
				self.y + math.floor(normal_font:getHeight() / 2),
				normal_font:getWidth("w"),
				normal_font:getHeight()
			)
		end
