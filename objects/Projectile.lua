...
function Projectile:new(area, x, y, opts)
	...
	if current_room.player.size_multiplier > 1 then
		self.s = p_size
			* (current_room.player.projectile_size_multiplier + ((current_room.player.size_multiplier - 1) / 1.5))
	else
	...
	elseif self.attack == "Homing" then
		...
		while keep_looking and tries < 20 do
			...
			if
				self.target
				and self.target.x < gw
				and self.target.x > 0
				and self.target.y < gh
				and self.target.y > 0
			then
				keep_looking = false
			end
	...
	if current_room.player.projectile_ninety_degree_change then
		local function startAngleChange()
			...
			local function changeAngleFirst()
				...
				self.timer:after(
					"ninety_degree_second",
					0.1 / current_room.player.projectile_angle_change_frequency_multiplier,
					changeAngleSecond
				)
			end
			self.timer:every(
				"ninety_degree_first",
				0.25 / current_room.player.projectile_angle_change_frequency_multiplier,
				changeAngleFirst
			)
		end
	...
	if current_room.player.wavy_projectiles then
		...
		local function doTheWave()
			self.timer:tween(
				0.25,
				self,
				{ r = self.r - current_room.player.projectile_waviness_multiplier * wave_dir * math.pi / 4 },
				"linear"
			)
		end
		self.timer:tween(
			0.25,
			self,
			{ r = self.r + current_room.player.projectile_waviness_multiplier * wave_dir * math.pi / 8 },
			"linear",
			doTheWave
		)
	...
	if current_room.player.fast_slow then
		...
		local function becomeSlow()
			self.timer:tween(
				...
				0.3,
				self,
				{ v = current_room.player.projectile_deceleration_multiplier * old_v / 2 },
				"linear"
			)
		end
		self.timer:tween(
			...
			0.2,
			self,
			{ v = 2 * self.v * current_room.player.projectile_acceleration_multiplier },
			"in-out-cubic",
			becomeSlow
		)
	end
	...
	if current_room.player.slow_fast then
		...
		local function becomeFast()
			self.timer:tween(
				...
				0.3,
				self,
				{ v = 2 * old_v * current_room.player.projectile_acceleration_multiplier },
				"linear"
			)
		end
		self.timer:tween(
			...
			0.2,
			self,
			{ v = current_room.player.projectile_deceleration_multiplier * self.v / 2 },
			"in-out-cubic",
			becomeFast
		)
	end
...
function Projectile:expire()
	return function()
		if
			self.attack ~= "2Split"
			and self.attack ~= "4Split"
			and self.attack ~= "Explode"
			and (self.mine or (current_room.player.projectiles_explode_on_expiration and not self.dont_explode))
		then
		...
		if
			self.attack == "Spin"
			and not self.proj_spawned
			and current_room.player.chances.spin_projectile_on_expiration_chance:next()
		then
...
function Projectile:update(dt)
	...
	if self.bounce and self.bounce > 1 then
		...
		if self.x < 0 then
			self:die({ wall_split_angle = 0 })
		end
		if self.y < 0 then
			self:die({ wall_split_angle = math.pi / 2 })
		end
		if self.x > gw then
			self:die({ wall_split_angle = -math.pi })
		end
		if self.y > gh then
			self:die({ wall_split_angle = -math.pi / 2 })
		end
...
function Projectile:draw()
	...
	if self.attack == "Homing" or self.attack == "2Split" or self.attack == "4Split" or self.attack == "Explode" then
		...
		if self.attack == "Spread" then
			love.graphics.setColor(color255To1(table.random(all_colors)))
		end
		if self.attack == "Bounce" then
			love.graphics.setColor(color255To1(table.random(default_colors)))
		end
		love.graphics.draw(self.mesh_left, self.x, self.y)
		love.graphics.setColor(color255To1(default_color))
		if self.attack == "Flame" then
